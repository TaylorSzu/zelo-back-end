...
const log = {
  info: (msg) =>
    console.log(`${getTimestamp()} ${chalk.blueBright("[INFO]")} ${msg}`),
  success: (msg) =>
    console.log(
      `${getTimestamp()} ${chalk.greenBright("[✅ SUCESSO]")} ${msg}`
    ),
  warn: (msg) =>
    console.log(
      `${getTimestamp()} ${chalk.keyword("orange")("[⚠️ AVISO]")} ${msg}`
    ),
  error: (msg) =>
    console.error(`${getTimestamp()} ${chalk.redBright("[❌ ERRO]")} ${msg}`),
  tableName: (modelName) =>
    console.log(
      `${getTimestamp()} ${chalk.yellowBright("📊 TABELA:")} ${chalk.bold(
        modelName
      )}`
    ),
  separator: () =>
